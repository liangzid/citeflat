% CLI status + dashboard side by side (Appendix D.2).
% White terminal cards. Elided library rows marked with ...
\begin{figure*}[tp!]
  \centering
  \begin{tikzpicture}[
    ...
    % ---- status ----
    \node[term, text width=0.46\textwidth] (st) {%
    ...
    \node[
      ...
      text=HoldInk,
      anchor=south west,
    ] at ([yshift=4pt]st.north west) {CLI · status};
    ...
    \node[
      ...
      text=HoldInk,
      anchor=south west,
    ] at ([yshift=4pt]db.north west) {CLI · dashboard};
  \end{tikzpicture}
  \caption[Library inventory and defense telemetry from the CAITLYN CLI.]{%
    Library inventory and defense telemetry from the CAITLYN CLI.
    The status listing is truncated with \texttt{...}.
    Counts reflect a development workstation after the evaluation campaigns
    in Section~\ref{sec:evaluation}.
  }
  \label{fig:appendix-cli-status-dashboard}
\end{figure*}
